...
/**
 * If it looks like Ordered, then it must be Ordered This specialized {@link OrderComparator}
 * performs comparisons similar to {@link org.springframework.core.annotation.AnnotationAwareOrderComparator},
 * but with some key differences.
 *
 * <p>1) the {@link Order} annotation is always checked first.
 *
 * <p>2) if the {@link Order} annotation is added to the bean producer method, it will also be
 * checked, and take precedence over the annotation at the bean class level. If multiple bean
 * producer methods contribute the same instance to the bean factory, the behavior is unspecified.
 *
 * <p>3) beans do not need to implement {@link Ordered}, they simply need to have a public method
 * with the signature {@code int getOrder()}, and that method will be used to also determine the
 * order value.
 */
public class QuackAnnotationAwareOrderComparator extends OrderComparator {
  ...
  /**
   * Creates a new {@code QuackAnnotationAwareOrderComparator}, using the provided bean factory to
   * check for {@link Order} annotations on bean producer methods.
   *
